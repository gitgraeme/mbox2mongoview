...
const ThreadView: React.FC<ThreadViewProps> = ({ thread, collapsed, onToggleCollapse }) => {
  ...
  return (
    <>
      <Box sx={{ p: 2, borderBottom: '1px solid', borderColor: 'grey.200', bgcolor: 'grey.50', minHeight: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box>
        ...
        <Tooltip title="Print this thread">
          <IconButton aria-label="Print thread" onClick={() => window.print()} size="small" sx={{ p: 0.5, ml: 1 }}>
            <span role="img" aria-label="Print" style={{ fontSize: 18 }}>🖨️</span>
          </IconButton>
        </Tooltip>
      </Box>
